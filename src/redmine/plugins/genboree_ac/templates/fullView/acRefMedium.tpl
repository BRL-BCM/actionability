<%
  ...
  if(@referenceCategory == 'PMID')
    ...
    if(matchIdx)
%>
      <div id="ref_%<%= @reference %>%" class="ref table section pmid <%= @reference %>">
        <span class="row">
          <span class="refNum cell">
          ...
          <span class="cell">
            ...
            <% if(volIssue) then %> <span class="volIssue"><span class="value"><%= volIssue %></span><%= ':' if(pages) %></span><% end %>
...
<%  else %>
    ...
    %>
      <div id="ref_%<%= @reference %>%" class="ref table section pmid broken <%= @reference %> row">
        <span class="row">
          <span class="refNum cell">
            <span class="idx"><%= @__opts[:count] %>.</span>
          </span>
...
<% elsif(@referenceCategory == 'OMIM') %>
    ...
    %>
    <div id="ref_%<%= @reference %>%" class="ref table section omim <%= @reference %>">
      <span class="row">
        <span class="refNum cell">
          <span class="idx"><%= @__opts[:count] %>.</span>
        </span>
        <span class="cell">
          <span class="journal">Online Medelian Inheritance in Man, OMIM<span class="sup">&reg;</span>.</span> <span class="static">Johns Hopkins University, Baltimore, MD. MIM:</span>
            ...
            <!-- [missing] span class="date">{Date last edited} span -->
...
<% elsif( pv('Reference.Category') == 'Orphanet' ) %>
    <div id="ref_%<%= @reference %>%" class="ref table section orphanet <%= @reference %>">
      <span class="row">
        <span class="refNum cell">
          <span class="idx"><%= @__opts[:count] %>.</span>
        </span>
        <span class="citation cell">
...
<% elsif(@referenceCategory == 'GeneReview') %>
    ...
    %>
    <div id="ref_%<%= @reference %>%" class="ref table section geneReview <%= @reference %>">
        <span class="row">
          <span class="refNum cell">
            <span class="idx"><%= @__opts[:count] %>.</span>
          </span>
...
<% else %>
    <div id="ref_%<%= @reference %>%" class="ref table section other <%= @reference %>">
      <span class="row">
        <span class="refNum cell">
          <span class="idx"><%= @__opts[:count] %>.</span>
        </span>
