<%
  require 'time'
  refIdStr = pv('Reference')
  refCatStr = pv('Reference.Category')

  if( refCatStr == 'PMID')
    #matchIdx = ( @referencePmidCitation =~ /^([^,]+(?:,[^,]+)*)\.\s+(.+)\.\s+([^\.]+)\.\s+(\S+)(?:\s+[^;]+)?;([^:\.]+)(?:\s*:\s*([^\.]+))?/ )
    citationStr = pv('Reference.PMID.Citation')
    matchIdx = ( citationStr =~ /^([^,]+(?:,[^,]+)*)\.\s+(.+)\.\s+([^\.]+)\.\s+([^\s;\.]+)(?:(\s+[^;]+)?;([^:\.]+)(?:\s*:\s*([^\.]+))?)?/ )
    if(matchIdx)
      #$stderr.puts "    1st - #{$~.captures.inspect}\n\n"
      authors, title, journal, year, date, volIssue, pages = $~.captures
    elsif(matchIdx = ( citationStr =~ /^([^,]+(?:,[^,]+)*)\.\s+(.+)\.\s+([^\.]+)\.\s+(\S+)\s+([^;]+)/ ) )
      #$stderr.puts "    2nd - #{$~.captures.inspect}\n\n"
      authors, title, journal, year, date, volIssue, pages = $~.captures
    elsif(matchIdx = ( citationStr =~ /^(.+)\.\s+([^\.]+)\.\s+([^\s;\.]+)(?:(?:\s+[^;]+)?;([^:\.]+)(?:\s*:\s*([^\.]+))?)?/ ) )
      #$stderr.puts "    3rd - #{$~.captures.inspect}\n\n"
      title, journal, year, volIssue, pages = $~.captures
    else
      #$stderr.puts "    NONE!"
      matchIdx = nil
    end
    ...
    if(matchIdx)
      volIssuePages = ''
      unless( volIssue.blank? )
        volIssuePages << volIssue
        volIssuePages << ':' unless( pages.blank? )
      end
      unless( pages.blank? )
        volIssuePages << pages
      end
      unless( volIssuePages.blank? )
        volIssuePages << '.'
      end
%>
      <div id="ref_%<%= refIdStr %>%" class="ref section pmid <%= refIdStr %> row">
        ...
        <span class="citation col-xs-11">
          ...
          <% if( !volIssuePages.blank? ) then %> <span class="volIssue pages"><span class="value"><%= volIssuePages %></span></span><% end %>
          <a class="link fa fa-external-link" target="_blank" href="http://www.ncbi.nlm.nih.gov/pubmed/?term=<%= pv('Reference.PMID') %>"></a>
        </span>
      </div>
<%  else %>
      ...
      %>
        <div id="ref_%<%= refIdStr %>%" class="ref section pmid broken <%= refIdStr %> row">
          <span class="refNum text-right col-xs-1">
            <span class="idx"><%= @__opts[:count] %>.</span>
          </span>
...
<% elsif( refCatStr == 'OMIM' ) %>
    ...
    %>
    <div id="ref_%<%= refIdStr %>%" class="ref section omim <%= refIdStr %> row">
      <span class="refNum text-right col-xs-1">
        <span class="idx"><%= @__opts[:count] %>.</span>
      </span>
      <span class="citation col-xs-11">
        <span class="journal">Online Medelian Inheritance in Man, OMIM<span class="sup">&reg;</span>.</span> <span class="static">Johns Hopkins University, Baltimore, MD. </span>
        <span class="title"><%= pv('Reference.OMIM.Title').to_s.capitalize %>.</span>
        MIM: <span class="articleId"><a target="_blank" href="http://www.omim.org/entry/<%= pv('Reference.OMIM') %>"><%= pv('Reference.OMIM') %></a><%= dateUpdated ? ':' : '.' %></span>
...
<% elsif( refCatStr == 'GeneReview' ) %>
    ...
    %>
    <div id="ref_%<%= refIdStr %>%" class="ref section geneReview <%= refIdStr %> row">
      <span class="refNum text-right col-xs-1">
        <span class="idx"><%= @__opts[:count] %>.</span>
      </span>
      <span class="citation col-xs-11">
...
<% elsif( refCatStr == 'Orphanet' ) %>
    <div id="ref_%<%= refIdStr %>%" class="ref section orphanet <%= refIdStr %> row">
      <span class="refNum text-right col-xs-1">
        <span class="idx"><%= @__opts[:count] %>.</span>
      </span>
      <span class="citation col-xs-11">
...
<% else %>
    <div id="ref_%<%= refIdStr %>%" class="ref section other <%= refIdStr %> row">
      <span class="refNum text-right col-xs-1">
        <span class="idx"><%= @__opts[:count] %>.</span>
      </span>
      <span class="citation col-xs-11">
